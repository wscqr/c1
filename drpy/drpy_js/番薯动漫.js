...
async function _req(url, opt) {
    ...
    log(196)
        ...
        let cookie=typeof resp.headers["set-cookie"] == 'string'?[resp.headers["set-cookie"]]:resp.headers["set-cookie"];
        let ck=cookie.map(it => it.replace(/;.*/, "")).join(";");
        
        if(ck.includes('PHPSESSID')){
        rule.headers.cookie = ck;
        }
