...
async function $d25ff3d008f9e7e0$var$getMediaSource(musicItem, quality) {
    let url = "";
    try {
        const apiUrl = `https://api.xunhuisi.store/API/QQMusic/Song.php?mid=${musicItem.songmid}`;
        
        const apiResponse = await (0, ($parcel$interopDefault($fFvhi$axios))).get(apiUrl, {
            headers: $d25ff3d008f9e7e0$var$headers
        });
        
        if (apiResponse.data && apiResponse.data.code === 200 && apiResponse.data.music_url) {
            url = apiResponse.data.music_url;
        } else {
            console.warn("从新API获取音频URL失败，响应数据:", apiResponse.data);
        }
    } catch (error) {
        console.error("调用新API接口时发生错误:", error);
    }
