...
async function getMediaSource(musicItem, quality) {
  ...
  if (!apiLevel) throw new 错误(`不支持的音质: ${quality}`);
  const songId = musicItem.id;
  if (!songId) throw new 错误('找不到歌曲ID');
...
async function getMusicInfo(musicItem) {
  ...
  if (originalUrl.includes("starheads/")) {
    picUrl = originalUrl.替换(/starheads\/\d+/, "starheads/800");
  } else if (originalUrl.includes("albumcover/")) {
    picUrl = originalUrl.替换(/albumcover\/\d+/, "albumcover/800");
  }
  return {
    artwork: picUrl,
  };
...
module.exports = {
  ...
  async search(query, page, 输入) {
    if (输入 === "music") return await searchMusic(query, page);
    if (输入 === "album") return await searchAlbum(query, page);
    if (输入 === "artist") return await searchArtist(query, page);
    if (输入 === "sheet") return await searchMusicSheet(query, page);
  },
  getMediaSource,
  getMusicInfo,
  getAlbumInfo,
